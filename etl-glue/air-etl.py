...
def MyTransform(glueContext, dfc) -> DynamicFrameCollection:
    ...
    def parse_json_column(df_, json_columns):
        """
        ...
        df['uv'] = df['current'].apply(lambda x: x['uv'])
        df['co'] = df['current'].apply(lambda x: x['air_quality']['co'] )
        df['o3'] = df['current'].apply(lambda x: x['air_quality']['o3'] )
        df['so2'] = df['current'].apply(lambda x: x['air_quality']['so2'] )
        df['pm2_5'] = df['current'].apply(lambda x: x['air_quality']['pm2_5'] )
        df['pm10'] = df['current'].apply(lambda x: x['air_quality']['pm10'] )
        df['us-epa-index'] = df['current'].apply(lambda x: x['air_quality']['us-epa-index'] )
        df['gb-defra-index'] = df['current'].apply(lambda x: x['air_quality']['gb-defra-index'])
    ...
    df_pd = dfc_to_pd(dfc)
    df_pd = parse_json_column(df_pd, ['location', 'current', 'partition_0', 'partition_1', 'partition_2', 'partition_3'])
...
AWSGlueDataCatalog_node1736675916080 = glueContext.create_dynamic_frame.from_catalog(
    ...
    transformation_ctx="AWSGlueDataCatalog_node1736675916080")

# Script generated for node Custom Transform
CustomTransform_node1736675825335 = MyTransform(
    glueContext, 
    DynamicFrameCollection({"AWSGlueDataCatalog_node1736675916080": AWSGlueDataCatalog_node1736675916080}, glueContext))

# Script generated for node Select From Collection
SelectFromCollection_node1736675844320 = SelectFromCollection.apply(
    dfc=CustomTransform_node1736675825335, 
    key=list(CustomTransform_node1736675825335.keys())[0], transformation_ctx="SelectFromCollection_node1736675844320")
